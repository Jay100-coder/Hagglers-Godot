...
func _load_lose_screen_or_reload() -> void:
	if game_lost_scene:
		...
		_try_connecting_signal_to_node(instance, &"restart_pressed", _refresh)
...
func _refresh() -> void:
	...
	Global.amt_in_sales = 0
	Global.daily_goal = 500
	Global.day = 1
	Global.time = 120
	Global.hagglers.clear()
...
func _load_win_screen_or_ending() -> void:
	if game_won_scene:
		...
		canvas_layer.add_child(instance)
		get_tree().current_scene.add_child(canvas_layer)
		_try_connecting_signal_to_node(instance, &"continue_pressed", _continue)
		_try_connecting_signal_to_node(instance, &"restart_pressed", _refresh)
		_try_connecting_signal_to_node(instance, &"main_menu_pressed", _load_main_menu)
	else:
		_reload_level()
